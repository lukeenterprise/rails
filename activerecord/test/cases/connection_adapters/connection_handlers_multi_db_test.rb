    ...
    class ConnectionHandlersMultiDbTest < ActiveRecord::TestCase
      ...
      unless in_memory_db?
        ...
        def test_connects_to_using_top_level_key_in_two_level_config
          config = {
            "development" => { "adapter" => "sqlite3", "database" => "db/primary.sqlite3", "user" => "root" },
            "development_readonly" => { "adapter" => "sqlite3", "database" => "db/primary.sqlite3", "user" => "root_readonly" }
          }
          @prev_configs, ActiveRecord::Base.configurations = ActiveRecord::Base.configurations, config
      ...
      def test_calling_connected_to_on_a_non_existent_handler_raises
        ...
        assert_equal "No connection pool with 'some_role' role found for the '#{database_name}' database.", error.message
      end
