...
class LoadingTest < ActiveSupport::TestCase
  ...
  test "descendants loaded after framework initialization are cleaned on each request without cache classes" do
    ...
    initial = [ActiveStorage::Blob, ActiveStorage::Attachment, ActiveRecord::SchemaMigration, ActiveRecord::InternalMetadata, ApplicationRecord, "ActiveRecord::Base"].collect(&:to_s).sort
    ...
    get "/unload"
    assert_equal ["ActiveRecord::Base", "ActiveRecord::InternalMetadata", "ActiveRecord::SchemaMigration"], ActiveRecord::Base.descendants.collect(&:to_s).sort.uniq
  end
